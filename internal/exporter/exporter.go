...
func WriteSession(w io.Writer, idx *indexer.Indexer, sessionID string, format string, f Filters) (int, error) {
	...
	allowedType := func(t string) bool {
		...
		for _, v := range f.IncludeTypes {
			if t == strings.ToLower(strings.TrimSpace(v)) {
				return true
			}
		}
		return false
		...
		for _, m := range msgs {
			...
			case "dialog", "dialog_with_thinking":
				...
				if typ == "reasoning" {
					if includeThinking {
						if text != "" {
							dialogItems = append(dialogItems, dialogItem{Role: "assistant", Text: text, Type: "reasoning"})
						}
					}
					continue
				}
			...
			default:
				...
				if role == "user" || role == "assistant" {
					if text != "" {
						dialogItems = append(dialogItems, dialogItem{Role: role, Text: text})
					}
				}
			}
		}
...
func parseFuncCall(m *indexer.Message) (cmdLine string, argsDump string) {
	...
	if s, ok := args.(string); ok {
		...
		if json.Unmarshal([]byte(s), &obj) == nil {
			if cl := joinCommand(obj["command"]); cl != "" {
				return cl, ""
			}
	...
	if m2, ok := args.(map[string]any); ok {
		if cl := joinCommand(m2["command"]); cl != "" {
			return cl, ""
		}
		if b, err := json.MarshalIndent(m2, "", "  "); err == nil {
			return "", string(b)
		}
	}
	return "", ""
...
func parseFuncOutput(m *indexer.Message) (stdout string, stderr string) {
	...
	case string:
		...
		return t, ""
	case map[string]any:
		if s, _ := t["output"].(string); s != "" {
			stdout = s
		}
		if s, _ := t["stdout"].(string); s != "" {
			stdout = s
		}
		if s, _ := t["stderr"].(string); s != "" {
			stderr = s
		}
	}
	return stdout, stderr
